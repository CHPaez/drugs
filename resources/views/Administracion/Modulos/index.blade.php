    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $("#modulos-table tbody").on("change", ".submodulo", function() {
                ...
                // Vacía el selector submodulo actual
                $moduloSelect.empty();
                $.each(datos_Submodulo[0], function(index, item) {
                    if (id_submodulo == item.id) {
                        $("select[name^='modulo']").each(function() {
                            ...
                            if (!$moduloSelect.find(`option[value='${item.id}']`).length &&
        ...
        function selectores(proceso) {
            if (proceso == 'modulos') {
                $.ajax({
                    headers: {
                        'X-CSRF-TOKEN': "{{ csrf_token() }}"
                    },
                    url: "{{ route('Modulos.show', 'GET') }}",
                    dataType: 'json',
                    success: function(response) {
