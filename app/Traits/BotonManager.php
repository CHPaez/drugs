...
trait BotonManager
{
    ...
    protected $botones = [
        'crear' => "<button type='%s' class='btn btn-primary float-right m-1 %s' id='%s' name='%s'>Crear</button>",
        'guardar' => "<button type='%s' class='btn btn-primary float-left %s' id='%s' name='%s'>Guardar</button>",
        'actualizar' => "<button type='%s' class='btn btn-outline-primary btn-sm %s' id='%s' name='%s'><i class='mdi mdi-check'></i></button>",
        'editar' => "<button type='%s' class='btn btn-outline-success btn-sm %s' id='%s' name='%s'><i class='far fa-edit'></i></button>",
        'eliminar' => "<button type='%s' class='btn btn-outline-danger btn-sm %s' id='%s' name='%s'><i class='far fa-trash-alt'></i></button>",
    ];
    ...
    public function incluirBotones(object $usuario,array $incluir, $request): Array
    {
        ...
        foreach ($incluir as $boton => $items) {
            ...
            if (array_key_exists($boton, $this->botones) && $this->validarPermisos($boton)){
                ...
                $html['all'] .= sprintf($this->botones[$boton],$type,$class,$id,$name);
                $html[$boton] .= sprintf($this->botones[$boton],$type,$class,$id,$name);
            }
        }
