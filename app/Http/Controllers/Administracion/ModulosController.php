...
class ModulosController extends AppBaseController
{
     ...
     */
    public function getModulos($orderBy = ""){
        if(empty($orderBy)){
            $routes = modulos::select('id','MoNombre as modulo','MoSubmodulo as submodulo')
            ->get()->toArray();
    
        }else{
            $routes = modulos::selectRaw('MAX(id) as id, MoNombre as modulo')
            ->groupBy('MoNombre')
            ->get()
            ->toArray();
        }
    ...
    public function show($modalidad){
        if($modalidad == "individual"){
            $response = $this->getModulos();
        }else{
            $response = $this->getModulos("MoNombre");
        }
    ...
    public function acceso_general(int $id_rol,int $id_modulo){
        $nombre_modulo = Modulos::select('MoNombre')->where('id',25)->first();
        $match_modulos = Modulos::select('id')->where('MoNombre',$nombre_modulo->MoNombre)->get();
         
        foreach($match_modulos as $modulo){
            $datos = [
            ...
            Roles_Permisos::updateOrCreate(['Modulos_id' => $modulo->id],$datos);
